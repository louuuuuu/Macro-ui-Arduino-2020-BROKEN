...
#\_______/  \______/    \____/  \____/  \______/ \__|  \__|
            ...
            with open('template.txt','r+') as f:
                ...
                text =text.replace('def dfi():','def df'+str(maxe)+'():')
                text =text.replace('dfi()','df'+str(maxe)+'()')
                ...
                text =text.replace('i.delbutton = Button(frame, text="X",command=d2, width=5,style="C.TButton")', 'delbutton'+str(maxe)+' = Button(frame, text="X",command=d2, width=5,style='+"\""+str(maxe)+".TButton"+"\""+')')
                text =text.replace('i.delbutton = Button(frame, text="X",command=dfi, width=5,style="C.TButton")', 'delbutton'+str(maxe)+' = Button(frame, text="X",command=df'+str(maxe)+', width=5,style='+"\""+str(maxe)+".TButton"+"\""+')')
...
def func2():
    for i in serial.tools.list_ports.comports():
        comprint = (str(i).split(" ")[0])


    file = open('config.json','r+')
    json_dict = json.load(file)
    board = pyfirmata.Arduino(comprint)
    ...
    while True:
        ...
        if board.digital[4].read() is True:
            Button1filefolder = open("Macro/"+str(json_dict["CodeButton1"])+".txt", "r")
            b1read = Button1filefolder.read()
            file = open('config.json','r+')
            json_dict = json.load(file)
...
def restart():
    ...
    while True:
        ...
        file = open('config.json','r+')
        json_dict = json.load(file)
        if json_dict["restart"]==1:
            print("3")
            json_dict["CodeButton1"]=0
            json_dict["CodeButton2"]=0
            json_dict["CodeButton3"]=0
            json_dict["restart"]=0
            file.seek(0)
            json.dump(json_dict,file, indent=4)
            file.truncate()
